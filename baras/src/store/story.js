...
export let tellStory = (
  ...
  let response = await fetch("/apis/story/tell", {
    method: "POST",
    body: JSON.stringify({
      userId,
      title,
...
export let storyPage = (
  ...
  let response = await fetch(`/apis/story/${title}/cont`, {
    method: "PUT",
    body: JSON.stringify({
      userId,
      textMeasures,
...
export let getPages = (title, userId) => async (dispatch) => {
  let response = await fetch(`/apis/story/${title}/cont/${userId}`);
...
export let newPage = (
  ...
  let response = await fetch("/apis/story/:title/cont/newPage", {
    method: "POST",
    body: JSON.stringify({
      userId,
      textMeasures,
